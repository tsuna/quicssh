...
func TestIsFatalSessionError(t *testing.T) {
	tests := []struct {
		...
		{"graceful remote close (code 0)", &quic.ApplicationError{Remote: true, ErrorCode: 0, ErrorMessage: "client shutdown"}, true},
		{"remote close (code 1)", &quic.ApplicationError{Remote: true, ErrorCode: 1, ErrorMessage: "chaos disconnect"}, false},
		{"local close (code 0)", &quic.ApplicationError{Remote: false, ErrorCode: 0, ErrorMessage: "close"}, false},
